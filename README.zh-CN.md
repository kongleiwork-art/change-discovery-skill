...
3. 推荐低、中、高模型或推理档位，但不会自动切换模型。小改默认继续；中改和大改必须暂停，等待用户确认继续使用当前选择或完成调整。
4. 生成经过脱敏的检索摘要。
...
正确的开始方式应该是先判断改动规模，给出初步产品价值结论和改进方向，再推荐模型档位，而不是立即写代码。小改会说明继续使用当前选择；中改和大改会停在模型确认门，确认后才进行 GitHub 调研和深入需求分析。

在这个确认门中，Agent 必须区分已经给出的初步产品建议和模型档位建议，说明深入验证尚未开始，并给出一个清楚的动作：保留当前模型就回复“继续”，需要切换则切换后再回复“继续”。
