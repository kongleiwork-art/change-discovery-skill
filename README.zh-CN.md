...
2. 先初步判断需求是否必要、对产品和用户有什么价值、还缺少什么证据，并至少提出一个更小或更好的方向。
3. 若结论是**放弃**或**暂缓**，先停在产品决策门，不进入模型确认和深入调研。若结论是**继续**或**缩小范围**，再推荐低、中、高模型或推理档位，但不会自动切换模型。小改默认继续；中改和大改在继续/缩小范围时必须暂停，等待用户确认继续使用当前选择或完成调整。
4. 针对当前工作范围生成经过脱敏的检索摘要；若建议缩小范围，则按缩小后的方向检索。
5. 通过确认门后，中改和大改在比较方案前，先去 GitHub 调研类似实现。
...
正确的开始方式应该是先判断改动规模，给出初步产品价值结论和改进方向，而不是立即写代码。若结论是放弃或暂缓，应先停下，不进入模型选择和调研；若结论是继续或缩小范围，再推荐模型档位。小改会说明继续使用当前选择；中改和大改在继续/缩小范围时会停在模型确认门，确认后才进行 GitHub 调研和深入需求分析。

在模型确认门中，Agent 必须区分已经给出的初步产品建议和模型档位建议，说明深入验证尚未开始，并给出一个清楚的动作：保留当前模型就回复“继续”，需要切换则切换后再回复“继续”。若产品结论是放弃或暂缓，清楚的动作则是：仍要继续分析时回复“仍然继续”。

## 安全边界与限制

- 模型档位只是建议；Skill 不会自行切换模型或推理设置。
- 小改默认不增加模型确认轮次，除非用户明确要求所有改动都暂停。
- 放弃或暂缓会在模型选择和 GitHub 调研之前停下，除非用户明确表示仍然继续。
- 模型档位建议必须与“继续、缩小范围、暂缓或放弃”的初步产品建议明确分开。
- 模型确认门里的“继续”不等于批准修改项目文件。
